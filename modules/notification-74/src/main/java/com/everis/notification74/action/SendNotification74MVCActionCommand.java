package com.everis.notification74.action;

import com.everis.notification74.constants.CustomNotification74Type;
import com.everis.notification74.constants.CustomNotifications74Constants;
import com.everis.notification74.constants.Notification74PortletKeys;
import com.everis.notification74.custom.CustomNotification74Handler;
import com.everis.notification74.custom.CustomSubscription74Sender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification74PortletKeys.NOTIFICATION74,
...
public class SendNotification74MVCActionCommand implements MVCActionCommand {
	private static final Log logger = LogFactoryUtil.getLog(SendNotification74MVCActionCommand.class);
	...
	protected void notifySubscribers(HashSet<User> userList, long userId, ServiceContext serviceContext)
		...
		CustomNotification74Handler.setTitleKey(title);
		CustomNotification74Handler.setBodyKey(body);

		CustomSubscription74Sender subscriptionSender = new CustomSubscription74Sender();
		/**
		 * Set up the EMAIL TYPE custom notification
		 */
		String entryTitle = "Admin User Login";

		String fromName = PropsUtil.get(CustomNotifications74Constants.EMAIL_FROM_NAME);
		String fromAddress = GetterUtil.getString(PropsUtil.get(CustomNotifications74Constants.EMAIL_FROM_ADDRESS),
				PropsUtil.get(PropsKeys.ADMIN_EMAIL_FROM_ADDRESS));

		subscriptionSender.setClassPK(0);
		subscriptionSender.setClassName(SendNotification74MVCActionCommand.class.getName());
		...
		int notificationType = CustomNotification74Type.NOTIFICATION_TYPE_USER_74;

		subscriptionSender.setNotificationType(notificationType);

		String portletId = Notification74PortletKeys.NOTIFICATION74;
		...
		subscriptionSender.addPersistedSubscribers(SendNotification74MVCActionCommand.class.getName(), 0);
