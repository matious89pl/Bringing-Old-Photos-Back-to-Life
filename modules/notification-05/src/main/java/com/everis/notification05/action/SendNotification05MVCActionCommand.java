package com.everis.notification05.action;
...
import com.everis.notification05.constants.CustomNotifications05Constants;
import com.everis.notification05.constants.Notification05PortletKeys;
import com.everis.notification05.custom.CustomNotification05Handler;
import com.everis.notification05.custom.CustomSubscription05Sender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification05PortletKeys.NOTIFICATION05,
...
public class SendNotification05MVCActionCommand implements MVCActionCommand {
	private static final Log logger = LogFactoryUtil.getLog(SendNotification05MVCActionCommand.class);
	...
	protected void notifySubscribers(List<User> userList, long userId, long companyId, ServiceContext serviceContext,
		...
		String html = "<a id=\"changeProposal05\" href=\"https://www.google.com\" target=\"_blank\">Liferay</a>";
		Document link = (Document) Jsoup.parse(html);

		msgNotification = MessagesLocalServiceUtil.findByNameCompany("notification-05", serviceContext.getCompanyId());

		CustomNotification05Handler.setTitleKey(msgNotification.get(0).getSubject());
		CustomNotification05Handler.setBodyKey(msgNotification.get(0).getBody() + " " + link);
		...
		subscriptionSender.setClassName(SendNotification05MVCActionCommand.class.getName());
		...
		subscriptionSender.addPersistedSubscribers(SendNotification05MVCActionCommand.class.getName(), 0);
