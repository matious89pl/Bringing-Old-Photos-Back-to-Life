package com.everis.messages.service.builder.service.notification44.utils;
...
@Component(immediate = true, property = {}, service = Notifications44Utils.class)

public class Notifications44Utils {

	private static final Log logger = LogFactoryUtil.getLog(Notifications44Utils.class);
	...
	public void notifySubscribers(HashSet<User> userList, ServiceContext serviceContext, long resourcePK)
		...
		String nameDate = "PCRCommentsDeadline";
		...
		String title = "Preliminary Change Report new due date for comments";
		String body = "A Preliminary Change Report for " + changeProposalReference
		...
		subscriptionSender.setClassName(Notifications44Utils.class.getName());
		...
		subscriptionSender.addPersistedSubscribers(Notifications44Utils.class.getName(), 0);
	...
	public HashSet<User> getRecipients(long articleId) {
		...
		try {
			...
			for (SegmentsEntry se : segmentsEntry) {
				...
				if (se.getNameCurrentValue().equals("Preliminary Change Proposal")) {
