package com.everis.messages.service.builder.service.notification08.utils;
...
@Component(immediate = true, property = {}, service = Notifications08Utils.class)

public class Notifications08Utils {

	private static final Log logger = LogFactoryUtil.getLog(Notifications08Utils.class);

	public void notifySubscribers(List<User> userList, long userId, long companyId, ServiceContext serviceContext,
			String classPK) throws PortalException {

		/**
		 * Set up the WEBSITE TYPE custom notification by name
		 */

		recFormArticle article = recFormArticleLocalServiceUtil.getrecFormArticle(Long.parseLong(classPK));
		...
		try {
			...
			String fieldName = "ChangeProposalReference";
			Node node = document
					.selectSingleNode("//root//dynamic-element[@name='" + fieldName + "']//dynamic-content");
			String cpReference = node.getText();
			...
			title = "Change Proposer has Withdrawn from Change Proposal";
			body = cpReference + " has been withdrawn by Change Proposer " + link;
		...
		subscriptionSender.setClassName(Notifications08Utils.class.getName());
		...
		subscriptionSender.setMailId("custom_notitication08 to stake holder", 0);
		...
		subscriptionSender.addPersistedSubscribers(Notifications08Utils.class.getName(), 0);
		...
		try {
			...
			for (User user : userList) {
				...
				subscriptionSender.sendNotification08(user);
			}
	...
	public List<User> getRecipients(long companyId, long groupId, List<String> roleNames) {
		...
		try {
			...
			if (recipients.isEmpty()) {
				Role roleStakeholder = RoleLocalServiceUtil.getRole(companyId, "REC_Stakeholder");
				recipients.addAll(UserLocalServiceUtil.getRoleUsers(roleStakeholder.getRoleId()));
				Role roleManagement = RoleLocalServiceUtil.getRole(companyId, "Change_Management_Team");
				recipients.addAll(UserLocalServiceUtil.getRoleUsers(roleManagement.getRoleId()));
			}
