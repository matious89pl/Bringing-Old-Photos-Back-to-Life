package com.everis.messages.service.builder.service.notification11.utils;
...
@Component(immediate = true, property = {}, service = Notifications11Utils.class)

public class Notifications11Utils {

	private static final Log logger = LogFactoryUtil.getLog(Notifications11Utils.class);

	public void notifySubscribers(HashSet<User> userList, ServiceContext serviceContext, long articleId)
			throws PortalException {

		logger.debug("notifySubscribers... Notifications11");
		...
		try {
			...
			title = "Consultation has been released for comments";
			body = "A Consultation related to a " + consultationTitle + " has been commissioned. Go to the " + link
					+ " to view the Consultation. Closing date for responses is " + formattedDate + ".";
		...
		subscriptionSender.setClassName(Notifications11Utils.class.getName());
		...
		subscriptionSender.addPersistedSubscribers(Notifications11Utils.class.getName(), 0);
		...
		try {
			...
			for (User user : userList) {
				logger.debug("User from userList..." + user.getScreenName());
				subscriptionSender.sendNotification11(user);
			}
	...
	public HashSet<User> getRecipients(long articleId) {
		...
		logger.debug(" Users to send notification 11 to" + recipients.size());
		return recipients;
	}
}
