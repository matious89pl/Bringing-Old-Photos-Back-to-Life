package com.everis.messages.service.builder.service.notification71.utils;
...
@Component(immediate = true, property = {}, service = Notifications71Utils.class)

public class Notifications71Utils {

	private static final Log logger = LogFactoryUtil.getLog(Notifications71Utils.class);
	...
	public void notifySubscribers(HashSet<User> userList, ServiceContext serviceContext, long currentUserId,
		...
		String link = " <a style='color:#70ada3;' onclick='@LINK1'>" + siteName + "</a>";

		logger.debug("Subject + Body: " + title + " - " + body);

		title = "Training, Guidance and Surveys log - Pending";
		body = activityName + " has been updated to 'Pending' by " + link;

		String linkEmail = "<a style='color:#70ada3' href='" + HtmlUtil.escape(urlEmail.concat(siteNameLinkToLowerCase))
				+ "'>" + HtmlUtil.escape(siteName) + "</a>";

		bodyEmail = activityName + " has been updated to 'Pending' by " + linkEmail;
		...
		subscriptionSender.setClassName(Notifications71Utils.class.getName());
		...
		subscriptionSender.addPersistedSubscribers(Notifications71Utils.class.getName(), 0);
