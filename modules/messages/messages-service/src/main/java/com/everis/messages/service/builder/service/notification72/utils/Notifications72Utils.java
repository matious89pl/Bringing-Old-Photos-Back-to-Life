...
@Component(immediate = true, property = {}, service = Notifications72Utils.class)

public class Notifications72Utils {

	private static final Log logger = LogFactoryUtil.getLog(Notifications72Utils.class);

	public void notifySubscribers(HashSet<User> userList, ServiceContext serviceContext, long currentUserId,
			long siteGroupId, String activityName, String urlEmail) throws PortalException {
		...
		String link = " <a style='color:#72ada3;' onclick='@LINK1'> performance assurance area </a>";

		logger.debug("Subject + Body: " + title + " - " + body);

		title = "Training, Guidance and Surveys log - Re-opened";
		body = activityName
				+ " has been re-opened on your organisations Training, Guidance and Surveys log. Please navigate to your organisation"
				+ HtmlUtil.escape("'") + "s " + link + " for more details";
		...
		bodyEmail = activityName
				+ " has been re-opened on your organisations Training, Guidance and Surveys log. Please navigate to your organisation"
				+ HtmlUtil.escape("'") + "s " + linkEmail + " for more details";
		...
		subscriptionSender.setClassName(Notifications72Utils.class.getName());
		...
		subscriptionSender.addPersistedSubscribers(Notifications72Utils.class.getName(), 0);
