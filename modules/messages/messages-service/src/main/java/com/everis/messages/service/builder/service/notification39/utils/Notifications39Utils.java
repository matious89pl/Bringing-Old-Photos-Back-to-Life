...
@Component(immediate = true, property = {}, service = Notifications39Utils.class)

public class Notifications39Utils {

	private static final Log logger = LogFactoryUtil.getLog(Notifications39Utils.class);
	...
	public void notifySubscribers(HashSet<User> userList, long userId, long resourcePrimaryKey, long siteGroupId,
		...
		logger.info("Notify Subscribers - Notification39 - resourcePrimaryKey " + resourcePrimaryKey);
		...
		try {
			...
			title = "Nominations Process Closed";
			body = "The Nominations Process for has now closed. " + link + ". Thank you for your participation.";

		} catch (Exception e1) {
			logger.error("Error notitication 39... " + e1);
		}
		...
		subscriptionSender.setClassName(Notifications39Utils.class.getName());
		...
		subscriptionSender.setMailId("custom_notitication39 to Responsible Service Provider(organisation)", 0);
		...
		subscriptionSender.addPersistedSubscribers(Notifications39Utils.class.getName(), 0);
	...
	/**
	 * Recipients Unique who has an specific role
	 *
	 * @param companyId
	 * @param groupId
