...
<%-- Commented by last requirements for the task 3500. Code: when the site is PAB the period perfromance custom field has been modified
<%
boolean isPFAsite = false;
long companyId = themeDisplay.getCompanyId();
long currentGroupId = themeDisplay.getScopeGroupId();

Group pfaSite = GroupLocalServiceUtil.fetchGroup(companyId, "Performance Assurance Board");

if(pfaSite !=null){
	long pfaSiteId = pfaSite.getGroupId();
	if (currentGroupId == pfaSiteId){
		isPFAsite = true;
	} 
}
...
<c:if test="<%= isPFAsite %>">
	<script>
		$(document).ready(function () {
			...
			$(".lfr-input-time").hide();
			$(".lfr-input-date .form-control").focusout(function() {
				  var dateNoChanges = $(".lfr-input-date .form-control")[0].value;
					if (dateNoChanges != "01/01/2000"){
						$(".btn-primary").attr("disabled", false);
					}
			});		
			$(".lfr-input-date .form-control").change(function() {
				  var dateNoChanges = $(".lfr-input-date .form-control")[0].value;
					if (dateNoChanges != "01/01/2000"){
						$(".btn-primary").attr("disabled", false);
					}
			});		
			var requiredSignal = $(".lfr-ddm-field-group .control-label")[1];
			requiredSignal.innerHTML = "Performance Period *";
		});	
	</script>
</c:if>
--%>
