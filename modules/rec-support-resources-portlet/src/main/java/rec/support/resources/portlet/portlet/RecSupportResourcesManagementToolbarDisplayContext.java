...
import rec.supporting.resources.service.supportRLocalServiceUtil;
 ...
 * @author liferay
 */
public class RecSupportResourcesManagementToolbarDisplayContext extends SearchContainerManagementToolbarDisplayContext {

	public RecSupportResourcesManagementToolbarDisplayContext(HttpServletRequest httpServletRequest,
	...
	public String getSearchActionURL() {
		...
		searchURL.setProperty("mvcRenderCommandName", RecSupportResourcesPortletKeys.VIEW_LANDING);
	...
	public String getDisplayStyle() {
		...
		if (Validator.isNull(displayStyle)) {
			...
			_portalPreferences.setValue(RecSupportResourcesPortletKeys.RECSUPPORTRESOURCES, "assignments-display-style",
					displayStyle);
	...
	private PortletURL _getCurrentSortingURL() throws PortletException {
		...
		sortingURL.setParameter("mvcRenderCommandName", RecSupportResourcesPortletKeys.VIEW_LANDING);
		// Reset current page.
	...
	/*
	 * @Override protected String getNavigation() { String navigation =
	 * ParamUtil.getString(liferayPortletRequest, getNavigationParam(), "all");
	 * String orderByType = ParamUtil.getString(liferayPortletRequest,
	 * getOrderByTypeParam(), "asc"); String orderByCol =
	 * ParamUtil.getString(liferayPortletRequest, getOrderByColParam(),
	 * "[myDefaultOrder]"); String keywords =
	 * ParamUtil.getString(liferayPortletRequest, "keywords", null); String
	 * displayStyle = ParamUtil.getString(liferayPortletRequest, "displayStyle",
		...
		return new ViewTypeItemList(portletURL, getDisplayStyle()) {
			{
				...
				/*
				 * add( viewTypeItem -> { viewTypeItem.setIcon("list");
				 * viewTypeItem.setHref(portletURL, "displayStyle", "descriptive");
				 * viewTypeItem.setLabel(LanguageUtil.get(LocaleUtil.getMostRelevantLocale(),
				 * "list")); viewTypeItem.setActive(Objects.equals(getDisplayStyle(),
				 * "descriptive")); });
				 */
			}
		};
	...
	public List<DropdownItem> getFilterDropdownItems() {
		...
		String portletConfigAggregateView = preferences.getValue("aggregateSupportingResourcesView", "no");
		...
		try {
			DynamicQuery dynamicQueryStatus = supportRLocalServiceUtil.dynamicQuery();
			...
			maintenanceStatusList = supportRLocalServiceUtil.dynamicQuery(dynamicQueryStatus, -1, -1);
		...
		if (StringUtil.equals(portletConfigAggregateView.toLowerCase(), "yes")) {
			try {
				DynamicQuery dynamicQueryParty = supportRLocalServiceUtil.dynamicQuery();
				List<String> maintenancePartyList = new ArrayList<String>();
				dynamicQueryParty
						.setProjection(ProjectionFactoryUtil.distinct(ProjectionFactoryUtil.property("specificParty")));
				// we filter specifiv partis by scopeGroupdId
				Order order = OrderFactoryUtil.asc("specificParty");
				dynamicQueryParty.addOrder(order);
				maintenancePartyList = supportRLocalServiceUtil.dynamicQuery(dynamicQueryParty, -1, -1);
	...
	public List<DropdownItem> getFilterDropdownItems(List<String> filterByStatus, List<String> filterByParty,
	...
	public DropdownItemList addDropdowFilterDueDateMenu(List<String> filterList) throws PortletException {
		...
		for (int i = 0; i < filterList.size(); i++) {
			...
			if (StringUtil.equalsIgnoreCase(filter, ParamUtil.getString(httpServletRequest, "filterStatus"))) {
				dropdownItem.setHref(_getCurrentSortingURL(), "filterDueDate", "none", "redirect",
						currentURLObj.toString());
			} else {
				dropdownItem.setHref(_getCurrentSortingURL(), "filterDueDate", filter, "redirect",
						currentURLObj.toString());
			}
			dropdownItem.setActive(
					StringUtil.equalsIgnoreCase(filter, ParamUtil.getString(httpServletRequest, "filterStatus")));
	 ...
	 * //TODO SE TIENE Q GUARDAR LA INFO EN UNA VARIABLE PARA SABER POR QUE SE ESTA
	 * FILTRANDO return new LabelItemList() { { add( labelItem -> {
