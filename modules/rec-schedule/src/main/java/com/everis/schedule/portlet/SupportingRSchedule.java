...
@Component(immediate = true, property = { "cron.expression= 0 0 8 ? * MON,TUE,WED,THU,FRI *" // scheduler runs everyday
		// at 9 UK
}, service = SupportingRSchedule.class)

public class SupportingRSchedule extends BaseMessageListener {

	private static final Log logger = LogFactoryUtil.getLog(SupportingRSchedule.class);
	private volatile boolean _initialized = false;

	@Override
	protected void doReceive(Message message) throws Exception {
		logger.info("Supporting Resources scheduled executed...");

		List<supportR> supportingROpen = supportRLocalServiceUtil.findByStatusOnly("Open");
		List<supportR> supportingRPending = supportRLocalServiceUtil.findByStatusOnly("Pending");

		List<supportR> supportingR = new ArrayList<supportR>();
		supportingR.addAll(supportingROpen);
		supportingR.addAll(supportingRPending);
		logger.debug("Supporting Resources iteration...");
		// iterate rfilogs open and pending to check due dates
		for (supportR suppR : supportingR) {
			Date suppDate = suppR.getDueDate();
			SimpleDateFormat fmt = new SimpleDateFormat("yyyyMMdd");
			logger.debug("Date from support resource item: " + suppDate);
			Date date = new Date();
			int businessDays = 10;
			Date nowPlus10 = addBussinessDays(date, businessDays);
			
			logger.debug("DueDate" + suppDate);
			logger.debug("nowPlus10" + nowPlus10);

			if (fmt.format(suppDate).equals(fmt.format(nowPlus10))) {				
				logger.debug("Due date encountered in 10 days...");
				try {
					logger.debug("Sending notification 74...");
					MessagesLocalServiceUtil.sendNotification74(suppR.getOrgSiteId(), suppR.getTitle(),
							suppR.getCompanyId());
				} catch (Exception e) {
					logger.error("Error sending notification 74 " + e);
				}
			}
			// check if is weekend
			...
			if (isWeekday) {
				...
				if ((dow == Calendar.MONDAY)) {
					...
					if (fmt.format(suppDate).equals(fmt.format(nowMinus3))) {
						logger.debug("Due date encountered...");
						try {
							logger.debug("Sending notification 73...");
							MessagesLocalServiceUtil.sendNotification73(suppR.getOrgSiteId(), suppR.getTitle(),
									suppR.getCompanyId());
						} catch (Exception e) {
							logger.error("Error sending notification 73 " + e);
						}
					}
				} else {
					...
					if (fmt.format(suppDate).equals(fmt.format(nowMinus1))) {
						logger.debug("Due date encountered...");
						try {
							logger.debug("Sending notification 73...");
							MessagesLocalServiceUtil.sendNotification73(suppR.getOrgSiteId(), suppR.getTitle(),
