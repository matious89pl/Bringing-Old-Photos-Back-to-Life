...
@Component(immediate = true, property = { "cron.expression= 0 0 8 ? * MON,TUE,WED,THU,FRI *" // scheduler runs everyday
																								// at 9 UK
}, service = ActivitySchedule.class)

public class ActivitySchedule extends BaseMessageListener {

	private static final Log logger = LogFactoryUtil.getLog(ActivitySchedule.class);
	private volatile boolean _initialized = false;

	@ProcessAction(name = "doReceive")
	protected void doReceive(Message message) throws Exception {
		logger.info("Activity scheduled executed...");
		
		List<MaintenanceActivity> activitiesOpen = MaintenanceActivityLocalServiceUtil.findByStatus("Open");
		List<MaintenanceActivity> activitiesPending = MaintenanceActivityLocalServiceUtil.findByStatus("Pending");

		List<MaintenanceActivity> activities = new ArrayList<MaintenanceActivity>();
		activities.addAll(activitiesOpen);
		activities.addAll(activitiesPending);
		logger.debug("Activities iteration...");

		// iterate activities open and pending to check due dates
		for (MaintenanceActivity maintenanceActivity : activities) {
			Date actvDate = maintenanceActivity.getDueDateFormated();
			...
			if (fmt.format(actvDate).equals(fmt.format(nowPlus10))) {				
				...
				try {
					logger.debug("Sending notification 54...");
					MessagesLocalServiceUtil.sendNotification54(maintenanceActivity.getOrgSiteId(),
							maintenanceActivity.getActivityTitle(), maintenanceActivity.getCompanyId());
				} catch (Exception e) {
					logger.error("Error sending notification 54 " + e);
				}
			}
			// check if is weekend
			...
			if (isWeekday) {
				...
				if ((dow == Calendar.MONDAY)) {
					...
					if (fmt.format(actvDate).equals(fmt.format(nowMinus3))) {
						...
						try {
							logger.debug("Sending notification 53...");
							MessagesLocalServiceUtil.sendNotification53(maintenanceActivity.getOrgSiteId(),
									maintenanceActivity.getActivityTitle(), maintenanceActivity.getCompanyId());
						} catch (Exception e) {
							logger.error("Error sending notification 53 " + e);
						}
					}
				} else {
