package com.everis.notification46.action;

import com.everis.notification46.constants.CustomNotification46Type;
import com.everis.notification46.constants.CustomNotifications46Constants;
import com.everis.notification46.constants.Notification46PortletKeys;
import com.everis.notification46.custom.CustomNotification46Handler;
import com.everis.notification46.custom.CustomSubscription46Sender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification46PortletKeys.NOTIFICATION46,
...
public class SendNotification46MVCActionCommand implements MVCActionCommand {
	private static final Log logger = LogFactoryUtil.getLog(SendNotification46MVCActionCommand.class);
	...
	private void _handleActionCommand(ActionRequest actionRequest) {
		...
		logger.info("Sending Custom Notification 46");
		...
		try {
			...
			String webEnabled = "Web";

			String emailEnabled = "Email";

			notifySubscribers(userList, user.getUserId(), serviceContext, webEnabled, emailEnabled);
	...
	protected void notifySubscribers(HashSet<User> userList, long userId, ServiceContext serviceContext,
			String webEnabled, String emailEnabled) throws PortalException {
		...
		CustomNotification46Handler.setTitleKey(title);
		CustomNotification46Handler.setBodyKey(body);
		...
		subscriptionSender.setClassName(SendNotification46MVCActionCommand.class.getName());
		...
		subscriptionSender.addPersistedSubscribers(SendNotification46MVCActionCommand.class.getName(), 0);
		...
		try {
			...
			for (User user : userList) {
				...
				subscriptionSender.sendNotification(user, webEnabled, emailEnabled);
