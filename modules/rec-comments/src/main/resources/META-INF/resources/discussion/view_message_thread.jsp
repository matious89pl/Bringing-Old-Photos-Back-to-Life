...
<%@page import="rec.confidential.message.model.Confidential_Messages"%>
<%@page import="rec.confidential.message.service.Confidential_MessagesLocalServiceUtil"%>
...
<%
	long messageId = discussionComment.getCommentId();
	boolean isConfidential = false;
	Confidential_Messages cnf_msg = Confidential_MessagesLocalServiceUtil.fetchConfidential_Messages(messageId);
	...
	<article class="lfr-discussion <%= (rootDiscussionComment.getCommentId() == discussionComment.getParentCommentId()) ? "lfr-discussion-container" : "" %>">
		<div class="comment-container">
			...
			>
				...
				<c:choose>
					<c:when test="<%= isConfidential %>">
						<span style="visibility: hidden;">NoImage</span>
					</c:when>
					<c:otherwise>
						<clay:content-col>
