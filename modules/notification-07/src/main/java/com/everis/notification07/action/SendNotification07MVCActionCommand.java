package com.everis.notification07.action;
...
import com.everis.notification07.constants.CustomNotifications07Constants;
import com.everis.notification07.constants.Notification07PortletKeys;
import com.everis.notification07.custom.CustomNotification07Handler;
import com.everis.notification07.custom.CustomSubscription07Sender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification07PortletKeys.NOTIFICATION07,
...
public class SendNotification07MVCActionCommand implements MVCActionCommand {
	private static final Log logger = LogFactoryUtil.getLog(SendNotification07MVCActionCommand.class);
	...
	protected void notifySubscribers(List<User> userList, long userId, long companyId, ServiceContext serviceContext,
		...
		msgNotification = MessagesLocalServiceUtil.findByNameCompany("notification-07", serviceContext.getCompanyId());

		CustomNotification07Handler.setTitleKey(msgNotification.get(0).getSubject());
		CustomNotification07Handler.setBodyKey(msgNotification.get(0).getBody());
		...
		String fromName = PropsUtil.get(CustomNotifications07Constants.EMAIL_FROM_NAME);
		String fromAddress = GetterUtil.getString(PropsUtil.get(CustomNotifications07Constants.EMAIL_FROM_ADDRESS),
				PropsUtil.get(PropsKeys.ADMIN_EMAIL_FROM_ADDRESS));

		CustomSubscription07Sender subscriptionSender = new CustomSubscription07Sender();

		subscriptionSender.setClassPK(0);
		subscriptionSender.setClassName(SendNotification07MVCActionCommand.class.getName());
		...
		subscriptionSender.setMailId("custom_notitication07", 0);

		int notificationType = CustomNotification07Type.NOTIFICATION_TYPE_USER_07;

		subscriptionSender.setNotificationType(notificationType);

		String portletId = Notification07PortletKeys.NOTIFICATION07;
		...
		subscriptionSender.addPersistedSubscribers(SendNotification07MVCActionCommand.class.getName(), 0);
