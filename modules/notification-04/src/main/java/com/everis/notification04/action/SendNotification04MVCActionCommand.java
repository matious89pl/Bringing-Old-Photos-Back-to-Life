package com.everis.notification04.action;
...
import com.everis.notification04.constants.CustomNotifications04Constants;
import com.everis.notification04.constants.Notification04PortletKeys;
import com.everis.notification04.custom.CustomNotification04Handler;
import com.everis.notification04.custom.CustomSubscription04Sender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification04PortletKeys.NOTIFICATION04,
...
public class SendNotification04MVCActionCommand implements MVCActionCommand {
	private static final Log logger = LogFactoryUtil.getLog(SendNotification04MVCActionCommand.class);
	...
	protected void notifySubscribers(List<User> userList, long userId, long companyId, ServiceContext serviceContext,
		...
		msgNotification = MessagesLocalServiceUtil.findByNameCompany("notification-04", serviceContext.getCompanyId());

		CustomNotification04Handler.setTitleKey(msgNotification.get(0).getSubject());
		CustomNotification04Handler.setBodyKey(msgNotification.get(0).getBody());
		...
		subscriptionSender.setClassName(SendNotification04MVCActionCommand.class.getName());
		...
		subscriptionSender.setMailId("custom_notitication04", 0);
		...
		subscriptionSender.addPersistedSubscribers(SendNotification04MVCActionCommand.class.getName(), 0);
