package com.everis.notification52.action;

import com.everis.notification52.constants.CustomNotification52Type;
import com.everis.notification52.constants.CustomNotifications52Constants;
import com.everis.notification52.constants.Notification52PortletKeys;
import com.everis.notification52.custom.CustomNotification52Handler;
import com.everis.notification52.custom.CustomSubscription52Sender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification52PortletKeys.NOTIFICATION52,
		"mvc.command.name=sendNotification" }, service = MVCActionCommand.class)

/**
 * class SendNotification52MVCActionCommand: the logic to send a custom
 * notifcation by email and website
 *
 * @author ccaravac
 */

public class SendNotification52MVCActionCommand implements MVCActionCommand {
	private static final Log logger = LogFactoryUtil.getLog(SendNotification52MVCActionCommand.class);
	...
	protected void notifySubscribers(HashSet<User> userList, long userId, ServiceContext serviceContext)
		...
		String title = "Title Notification 52";
		String body = "Body Notification 52";
		
		CustomNotification52Handler.setTitleKey(title);
		CustomNotification52Handler.setBodyKey(body);

		CustomSubscription52Sender subscriptionSender = new CustomSubscription52Sender();
		/**
		 * Set up the EMAIL TYPE custom notification
		 */
		String entryTitle = "Admin User Login";

		String fromName = PropsUtil.get(CustomNotifications52Constants.EMAIL_FROM_NAME);
		String fromAddress = GetterUtil.getString(PropsUtil.get(CustomNotifications52Constants.EMAIL_FROM_ADDRESS),
				PropsUtil.get(PropsKeys.ADMIN_EMAIL_FROM_ADDRESS));

		subscriptionSender.setClassPK(0);
		subscriptionSender.setClassName(SendNotification52MVCActionCommand.class.getName());
		...
		int notificationType = CustomNotification52Type.NOTIFICATION_TYPE_USER_52;

		subscriptionSender.setNotificationType(notificationType);

		String portletId = Notification52PortletKeys.NOTIFICATION52;
		...
		subscriptionSender.addPersistedSubscribers(SendNotification52MVCActionCommand.class.getName(), 0);
