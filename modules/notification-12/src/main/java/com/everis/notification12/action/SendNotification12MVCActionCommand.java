package com.everis.notification12.action;
...
import com.everis.notification12.constants.CustomNotifications12Constants;
import com.everis.notification12.constants.Notification12PortletKeys;
import com.everis.notification12.custom.CustomNotification12Handler;
import com.everis.notification12.custom.CustomSubscription12Sender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification12PortletKeys.NOTIFICATION12,
...
public class SendNotification12MVCActionCommand implements MVCActionCommand {
	private static final Log logger = LogFactoryUtil.getLog(SendNotification12MVCActionCommand.class);
	...
	private void _handleActionCommand(ActionRequest actionRequest) {
		...
		logger.info("Sending Custom Notification 12");
	...
	protected void notifySubscribers(List<User> userList, long userId, long companyId, ServiceContext serviceContext,
		...
		String html = "<a id=\"changeProposal12\" href=\"https://www.google.com\" target=\"_blank\">Liferay</a>";
		Document link = (Document) Jsoup.parse(html);

		msgNotification = MessagesLocalServiceUtil.findByNameCompany("notification-12", serviceContext.getCompanyId());

		CustomNotification12Handler.setTitleKey(msgNotification.get(0).getSubject());
		CustomNotification12Handler.setBodyKey(msgNotification.get(0).getBody());

		/**
		 * Set up the EMAIL TYPE custom notification
		 */
		String entryTitle = "Notification 12";

		String fromName = PropsUtil.get(CustomNotifications12Constants.EMAIL_FROM_NAME);
		String fromAddress = GetterUtil.getString(PropsUtil.get(CustomNotifications12Constants.EMAIL_FROM_ADDRESS),
				PropsUtil.get(PropsKeys.ADMIN_EMAIL_FROM_ADDRESS));

		CustomSubscription12Sender subscriptionSender = new CustomSubscription12Sender();

		subscriptionSender.setClassPK(0);
		subscriptionSender.setClassName(SendNotification12MVCActionCommand.class.getName());
		...
		subscriptionSender.setMailId("custom_notitication12", 0);

		int notificationType = CustomNotification12Type.NOTIFICATION_TYPE_USER_12;

		subscriptionSender.setNotificationType(notificationType);

		String portletId = Notification12PortletKeys.NOTIFICATION12;
		...
		subscriptionSender.addPersistedSubscribers(SendNotification12MVCActionCommand.class.getName(), 0);
