package com.everis.notification02.action;
...
import com.everis.notification02.constants.CustomNotificationsConstants;
import com.everis.notification02.constants.Notification02PortletKeys;
import com.everis.notification02.custom.CustomNotificationHandler;
import com.everis.notification02.custom.CustomNotificationType;
import com.everis.notification02.custom.CustomSubscriptionSender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification02PortletKeys.NOTIFICATION02,
		"mvc.command.name=sendNotification" }, service = MVCActionCommand.class)
public class SendNotification02MVCActionCommand implements MVCActionCommand {

	private static final Log logger = LogFactoryUtil.getLog(SendNotification02MVCActionCommand.class);
	...
	protected void notifySubscribers(List<User> userList, long userId, long companyId, ServiceContext serviceContext,
		...
		msgNotification = MessagesLocalServiceUtil.findByNameCompany("notification-02", serviceContext.getCompanyId());

		CustomNotificationHandler.setTitleKey(msgNotification.get(0).getSubject());
		CustomNotificationHandler.setBodyKey(msgNotification.get(0).getBody());
		...
		subscriptionSender.setClassName(SendNotification02MVCActionCommand.class.getName());
		...
		subscriptionSender.setMailId("custom_notitication01", 0);

		int notificationType = CustomNotificationType.NOTIFICATION_TYPE_USER_02;

		subscriptionSender.setNotificationType(notificationType);

		String portletId = Notification02PortletKeys.NOTIFICATION02;
		...
		subscriptionSender.addPersistedSubscribers(SendNotification02MVCActionCommand.class.getName(), 0);
