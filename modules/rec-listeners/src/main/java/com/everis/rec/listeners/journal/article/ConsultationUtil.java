...
public class ConsultationUtil {
	...
	private final static String CONSULTATION_QA_A = "CONSULTATION-QA-A";

	private final static String CONSULTATION_QA_B = "CONSULTATION-QA-B";
	...
	public void checkUpdatesAndApplyLogic(JournalArticle model) {
		...
		try {
			if (model.getVersion() != version) {
				...
				if (commentsDeadlineHasBeenUpdated(model)) {
					logger.info("CommentsDeadlineHasBeenUpdated - Sending Notification 10(listener) by Segments");
					String cpID = getNodeText(model, RecListenersPortletKeys.CONSULTATION_ID_CP_FIELDNAME);
					logger.debug("cpID: " + cpID + " - " + "model.getArticleId(): " + model.getArticleId());
					MessagesLocalServiceUtil.sendNotification10(model.getResourcePrimKey());
				...
				logger.info("New consultation created - sending notification 11 (listener)");
				MessagesLocalServiceUtil.sendNotification11(model.getResourcePrimKey());
				if (model.getStatus() == 0) {
					logger.info("Consultation approved");
		...
		String docFieldName = "TextDocumentConsultation";
		try {
			...
			logger.error("Error while reviewing Consultation docs permissions");
			e.printStackTrace();
		}
	}
	...
	private void createQAStructure(JournalArticle model) throws DocumentException {
		logger.info("Creating DDL Questions & Answers for Consultation: " + model.getResourcePrimKey());
		long structureId = getStructureIdBytemplateOption(model);
		if (Validator.isNotNull(structureId)) {
			Map<Locale, String> titleMap = new HashMap<>();
			String resourcePrimKey = String.valueOf(model.getResourcePrimKey());
			String title = "Consultation-".concat(resourcePrimKey);
			titleMap.put(Locale.ENGLISH, title);
			logger.debug("title: " + title);
			Map<Locale, String> descriptionMap = new HashMap<>();
			titleMap.put(Locale.ENGLISH, title);
			String recordSetKey = "CON".concat(String.valueOf(model.getResourcePrimKey()));
			...
			try {
				...
				logger.error("Error adding new DDL Record for Consultation: " + model.getResourcePrimKey());
			}
	...
	private long getStructureIdBytemplateOption(JournalArticle model) throws DocumentException {
		String templateOption = getNodeText(model, RecListenersPortletKeys.CONSULTATION_CHOOSEQATEMPLATE);
		long structureId = 0;
		logger.debug("templateOption: " + templateOption);
		DynamicQuery dynamicQuery = DDMStructureLocalServiceUtil.dynamicQuery();
		switch (templateOption) {
		case "A":
			dynamicQuery.add(PropertyFactoryUtil.forName("name").like("%" + CONSULTATION_QA_A + "%"));
			break;
		case "B":
			dynamicQuery.add(PropertyFactoryUtil.forName("name").like("%" + CONSULTATION_QA_B + "%"));
			break;
		default:
			logger.info("There is no template option selected for a Consultation");
		}
	...
	private boolean commentsDeadlineHasBeenUpdated(JournalArticle model) throws DocumentException, ParseException {
		...
		if (!isCustomUpdate(previousModel, model)) {
			...
			String previousCommentsDeadline = getNodeText(previousModel,
					RecListenersPortletKeys.CONSULTATION_RESPONSES_DEADLINE);
	...
	private boolean chooseQATemplateHasBeenUpdated(JournalArticle model) throws DocumentException, ParseException {
		...
		if (!isCustomUpdate(previousModel, model)) {
			...
			String previousChooseQATemplate = getNodeText(previousModel,
					RecListenersPortletKeys.CONSULTATION_CHOOSEQATEMPLATE);
			String currentChooseQATemplate = getNodeText(model, RecListenersPortletKeys.CONSULTATION_CHOOSEQATEMPLATE);
