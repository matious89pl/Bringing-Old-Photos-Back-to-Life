package com.everis.notification15.action;
...
import com.everis.notification15.constants.CustomNotifications15Constants;
import com.everis.notification15.constants.Notification15PortletKeys;
import com.everis.notification15.custom.CustomSubscription15Sender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification15PortletKeys.NOTIFICATION15,
...
public class SendNotification15MVCActionCommand implements MVCActionCommand {
	private static final Log logger = LogFactoryUtil.getLog(SendNotification15MVCActionCommand.class);
	...
	protected void notifySubscribers(List<User> userList, long userId, long companyId, ServiceContext serviceContext,
		...
		String html = "<a id=\"changeProposal15\" href=\"https://www.google.com\" target=\"_blank\">Liferay</a>";
		Document link = (Document) Jsoup.parse(html);

//		msgNotification = MessagesLocalServiceUtil.findByNameCompany("notification-15", serviceContext.getCompanyId());
		
//		CustomNotification15Handler.setTitleKey(msgNotification.get(0).getSubject());
//		CustomNotification15Handler.setBodyKey(msgNotification.get(0).getBody() + " " + link);
		
//		User userData = UserLocalServiceUtil.getUser(serviceContext.getUserId());
//		
//		String email = userData.getEmailAddress();
//
//		CustomNotification15Handler.setTitleKey("Tittle notification15");
//		CustomNotification15Handler.setBodyKey("Body notitication15 " + email);
		...
		subscriptionSender.setClassName(SendNotification15MVCActionCommand.class.getName());
		...
		subscriptionSender.setMailId("custom_notitication15", 0);
		...
		subscriptionSender.addPersistedSubscribers(SendNotification15MVCActionCommand.class.getName(), 0);
