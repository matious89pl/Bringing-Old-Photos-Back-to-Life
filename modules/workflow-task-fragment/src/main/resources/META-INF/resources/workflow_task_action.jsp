...
boolean hasCustomViewPermission = false;
long userId = themeDisplay.getUserId();
long workflowDefinitionId = workflowTask.getWorkflowDefinitionId();

KaleoDefinition kaleoDefinition = KaleoDefinitionLocalServiceUtil.getKaleoDefinition(workflowDefinitionId);
String workflowTitle = kaleoDefinition.getTitle();
String[] workflowNames = {"Change Proposal", "Operational Contacts Register", "Voluntary Withdrawal", "Web Content", "Nomination"};
for (String eachWorkflow : workflowNames) {
	if (workflowTitle.contains(eachWorkflow)) {
		List<WorkflowTaskAssignee> assigneeList = workflowTask.getWorkflowTaskAssignees();
		for(WorkflowTaskAssignee assignee : assigneeList){
			String className = assignee.getAssigneeClassName();
			long classPK = assignee.getAssigneeClassPK();
			if (className.equals("com.liferay.portal.kernel.model.Role")){
				User userSession = UserLocalServiceUtil.getUser(userId);
				List<Role> roles = userSession.getRoles();
				for (Role role : roles){
					if (classPK == role.getRoleId()){
						hasCustomViewPermission = true;
						break;
					}
				}
			} else if (className.equals("com.liferay.portal.kernel.model.User")){
				if (classPK == userId){
					hasCustomViewPermission = true;
					break;
				}
...
>
	<c:if test="<%= !workflowTask.isCompleted() %>">
		...
		<c:if test="<%= hasCustomViewPermission == true %>">
