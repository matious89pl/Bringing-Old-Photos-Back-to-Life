package com.everis.notification55.action;

import com.everis.notification55.constants.CustomNotification55Type;
import com.everis.notification55.constants.CustomNotifications55Constants;
import com.everis.notification55.constants.Notification55PortletKeys;
import com.everis.notification55.custom.CustomNotification55Handler;
import com.everis.notification55.custom.CustomSubscription55Sender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification55PortletKeys.NOTIFICATION55,
...
public class SendNotification55MVCActionCommand implements MVCActionCommand {
	private static final Log logger = LogFactoryUtil.getLog(SendNotification55MVCActionCommand.class);
	...
	protected void notifySubscribers(HashSet<User> userList, long userId, ServiceContext serviceContext)
		...
		String title = "RFI Log - Completed";
		String body = "Notification triggered when a RFI log is changed to complete status";

		CustomNotification55Handler.setTitleKey(title);
		CustomNotification55Handler.setBodyKey(body);

		CustomSubscription55Sender subscriptionSender = new CustomSubscription55Sender();
		/**
		 * Set up the EMAIL TYPE custom notification
		 */
		String entryTitle = "Admin User Login";

		String fromName = PropsUtil.get(CustomNotifications55Constants.EMAIL_FROM_NAME);
		String fromAddress = GetterUtil.getString(PropsUtil.get(CustomNotifications55Constants.EMAIL_FROM_ADDRESS),
				PropsUtil.get(PropsKeys.ADMIN_EMAIL_FROM_ADDRESS));

		subscriptionSender.setClassPK(0);
		subscriptionSender.setClassName(SendNotification55MVCActionCommand.class.getName());
		...
		int notificationType = CustomNotification55Type.NOTIFICATION_TYPE_USER_55;

		subscriptionSender.setNotificationType(notificationType);

		String portletId = Notification55PortletKeys.NOTIFICATION55;
		...
		subscriptionSender.addPersistedSubscribers(SendNotification55MVCActionCommand.class.getName(), 0);
