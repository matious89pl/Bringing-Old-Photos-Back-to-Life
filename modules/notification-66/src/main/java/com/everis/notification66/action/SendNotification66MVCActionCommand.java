package com.everis.notification66.action;

import com.everis.notification66.constants.CustomNotification66Type;
import com.everis.notification66.constants.CustomNotifications66Constants;
import com.everis.notification66.constants.Notification66PortletKeys;
import com.everis.notification66.custom.CustomNotification66Handler;
import com.everis.notification66.custom.CustomSubscription66Sender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification66PortletKeys.NOTIFICATION66,
...
public class SendNotification66MVCActionCommand implements MVCActionCommand {
	private static final Log logger = LogFactoryUtil.getLog(SendNotification66MVCActionCommand.class);
	...
	protected void notifySubscribers(HashSet<User> userList, long userId, ServiceContext serviceContext)
		...
		CustomNotification66Handler.setTitleKey(title);
		CustomNotification66Handler.setBodyKey(body);

		CustomSubscription66Sender subscriptionSender = new CustomSubscription66Sender();
		/**
		 * Set up the EMAIL TYPE custom notification
		 */
		String entryTitle = "Admin User Login";

		String fromName = PropsUtil.get(CustomNotifications66Constants.EMAIL_FROM_NAME);
		String fromAddress = GetterUtil.getString(PropsUtil.get(CustomNotifications66Constants.EMAIL_FROM_ADDRESS),
				PropsUtil.get(PropsKeys.ADMIN_EMAIL_FROM_ADDRESS));

		subscriptionSender.setClassPK(0);
		subscriptionSender.setClassName(SendNotification66MVCActionCommand.class.getName());
		...
		int notificationType = CustomNotification66Type.NOTIFICATION_TYPE_USER_66;

		subscriptionSender.setNotificationType(notificationType);

		String portletId = Notification66PortletKeys.NOTIFICATION66;
		...
		subscriptionSender.addPersistedSubscribers(SendNotification66MVCActionCommand.class.getName(), 0);
