...
<script>
	$(document).ready(function() {
		$(".notification061knowledgeRepositoryLink").each(function() {
		    $("<span> in the knowledge repository. If you want to discuss the rejection with a member of the Change Management Team, please raise a service request with the helpdesk at </span>").insertAfter($(this));
		});
		
// 		$(".notification16IAReportLink").each(function() {
// 			console.log($(this));
// 			var deadline = $(this).attr("data-commentsDeadline");
// 		    $("<span>, closing date for comments are " + deadline + "</span>").insertAfter($);
// 		});
	});
</script>

<script>
	function customMarkAsReadNotification(event, userId,
			userEventNotificationId, url) {

		Liferay.Service('/messages.messages/mark_as_read_notification', {
			userId : userId,
			userEventNotificationId : userEventNotificationId
		}, function(obj) {
			console.log("Notification custom clicked");
	...
	function markAsReadNotification(url) {
		window.location.href = url;
	}
</script>
