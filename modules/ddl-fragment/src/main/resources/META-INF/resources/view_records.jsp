...
String recordSetId = GetterUtil.getString(request.getParameter("recordSetId"));

String customSearchByTargetedAudience = GetterUtil.getString(request.getParameter("customSearch"));
...
<%	
	DDLRecordSet ddlRecordSet = ddlDisplayContext.getRecordSet();
	List<DDLRecord> filteredResults = new ArrayList<DDLRecord>();
	SearchContainer search = ddlViewRecordsDisplayContext.getSearch();
	List<Object> results = search.getResults();
	
	boolean hasAccessRole = false;
	if (RoleLocalServiceUtil.hasUserRole(themeDisplay.getUserId(), themeDisplay.getCompanyId(), "Change_Management_Team", false)) {
		hasAccessRole = true;
	}
	
	for(Object record : results){
		DDLRecord ddlRecord = (DDLRecord) record;
		if(ddlRecord.getUserId() == themeDisplay.getUserId() || hasAccessRole){
			filteredResults.add(ddlRecord);
		}
	}
	search.setResults(filteredResults);
	search.setTotal(filteredResults.size());
	
	SearchContainer finalSearch = null;
	String custom = "customSearch";
	if (customSearchByTargetedAudience.equalsIgnoreCase(custom)){
		finalSearch = search;
	} else {
		finalSearch = ddlViewRecordsDisplayContext.getSearch();
	}
%>

	<c:choose>
	  <c:when test="<%= ddlRecordSet.getName("en_GB").startsWith("APP-Activity-Log-") %>">
	  	<!-- ACTIVITY LOG DDL -->
	  	<%@ include file="/view_activity_log.jspf" %>
	  </c:when>
	  <c:otherwise>
	    <!-- NON ACTIVITY LOG DDL -->
