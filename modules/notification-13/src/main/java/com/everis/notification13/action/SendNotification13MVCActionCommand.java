package com.everis.notification13.action;
...
import com.everis.notification13.constants.CustomNotifications13Constants;
import com.everis.notification13.constants.Notification13PortletKeys;
import com.everis.notification13.custom.CustomNotification13Handler;
import com.everis.notification13.custom.CustomSubscription13Sender;
...
@Component(immediate = true, property = { "javax.portlet.name=" + Notification13PortletKeys.NOTIFICATION13,
...
public class SendNotification13MVCActionCommand implements MVCActionCommand {
	private static final Log logger = LogFactoryUtil.getLog(SendNotification13MVCActionCommand.class);
	...
	protected void notifySubscribers(List<User> userList, long userId, long companyId, ServiceContext serviceContext,
		...
		String html = "<a id=\"changeProposal13\" href=\"https://www.google.com\" target=\"_blank\">Liferay</a>";
		Document link = (Document) Jsoup.parse(html);

		msgNotification = MessagesLocalServiceUtil.findByNameCompany("notification-13", serviceContext.getCompanyId());

		CustomNotification13Handler.setTitleKey(msgNotification.get(0).getSubject());
		CustomNotification13Handler.setBodyKey(msgNotification.get(0).getBody() + " " + link);
		...
		subscriptionSender.setClassName(SendNotification13MVCActionCommand.class.getName());
		...
		subscriptionSender.addPersistedSubscribers(SendNotification13MVCActionCommand.class.getName(), 0);
