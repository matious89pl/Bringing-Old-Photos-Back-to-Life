...
<%@ page import="com.everis.cproposal.service.recFormArticleLocalServiceUtil" %>
...
if (calendarBooking != null) {
	...
	boolean isSiteMember2 = GroupLocalServiceUtil.hasUserGroup(themeDisplay.getUserId(), themeDisplay.getScopeGroupId());
	if(isSiteMember2){
		pendingCalendarBookings.clear();
	} else{
...
Boolean isSiteMemberGlobal = GroupLocalServiceUtil.hasUserGroup(themeDisplay.getUserId(), themeDisplay.getScopeGroupId());
...
<input name="isSiteMember" type="hidden" id="isSiteMember" value="<%=isSiteMemberGlobal %>"/>
<input name="rpaCalId" type="hidden" id="rpaCalId" value=""/>
...
<aui:form action="<%= updateFormCalendarBookingURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + liferayPortletResponse.getNamespace() + "updateCalendarBooking();" %>'>
	<aui:input name="mvcPath" type="hidden" value="/edit_calendar_bookings.jsp" />

	<liferay-portlet:renderURL var="redirectURL">
		<liferay-portlet:param name="mvcPath" value="/edit_calendar_bookings.jsp" />
				...
				>
					...
					>
						...
						<%
							...
							Boolean isSiteMember = GroupLocalServiceUtil.hasUserGroup(themeDisplay.getUserId(), themeDisplay.getScopeGroupId());
							if(isSiteMember || hasAccessRPA){
								%>
									<aui:button cssClass="inviteRPA" value="Invite Code Manager " name="inviteRPA"/>
								<% 
							} 
							if(hasAccessRPA){
								%>
	...
	Liferay.provide(
		...
		() => {
			...
			<c:if test="<%= invitable %>">
				...
				var isRECPartyUser = $("#isSiteMember").val();
				var calendarIdVal = $("#rpaCalId").val();
				if (isRECPartyUser == "true" && calendarIdVal != "") {
	...
	$(document).ready(function() {
		$(".inviteRPA").click(function() {
			<% 
					...
					%>	
					var calendar = <%= recFormArticleLocalServiceUtil.getCalendarJsonObject(themeDisplay,RPAcalendar)%> 
					if (
						...
						remoteServices.hasExclusiveCalendarBooking(
							...
							(result) => {
								if (result) {
									...
									$("#rpaCalId").val(<%=RPAcalendar.getCalendarId() %>);								
								}
								else {
									<portlet:namespace />calendarListPending.add(calendar);
									$("#rpaCalId").val(<%=RPAcalendar.getCalendarId() %>);
				...
				$("#rpaCalId").val(<%=RPAcalendar.getCalendarId() %>);	
							...
							console.log("here is after add list");
