...
String completeUrl = themeDisplay.getURLHome()+"/calendar-management";
...
for(Role rol : userRoles) {
	...
	if(rol.getDescriptiveName().equals("Administrator")){
		isAdminUser = true;
	    break;
	}
}
...
long id = 0;
Calendar cal = CalendarLocalServiceUtil.fetchGroupCalendar(themeDisplay.getCompanyId(), themeDisplay.getScopeGroupId(), "Upcoming_Events");
if(cal != null) {
	...
	<clay:row>
		<c:if test="<%= !displaySchedulerOnly %>">
			...
			<h1>Calendar</h1>
				...
				<c:if test="<%=isSignedIn && isAdminUser%>">
...
Liferay.on(
	...
	*/
	...
	function() {
		...
		Liferay.Service(
		  '/cproposal.recformarticle/get_calendar_id_UE',
		  {
		  ...
		  function(obj) {
			  idCalendarHome = obj;
			  console.log("idCalendarHome " + idCalendarHome);
		  }
		);
		...
		setTimeout(function(){ 
			...
			$("#home-calendar td").click(function (e) {
			$("#home-calendar td").removeClass("selected-day lfr-current-day");
			e.stopPropagation();
...
function hideEmptyEvents(id) {
	console.log("Inside hideEmptyEvents with id " + id);
