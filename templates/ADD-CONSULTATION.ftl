<!-- PA. Form upload file -->
    ...
    <#else>
    <#assign edit = false />
    <#assign assetURL = "/group/guest/consultation-register" />
    </#if>

       <#assign ED_Consultation_Title = ""/>
            <#assign ED_Consultation_CPID = ""/>
            <#assign ED_Consultation_TargetedAudience = ""/>

            <#assign ED_Consultation_ChooseQATemplate = "" />
            <#assign ED_Consultation_LinkToCPPage = ""/>
            <#assign ED_Consultation_Documents = "" />

            <#assign ED_Consultation_Document_Title = "" />
            <#assign ED_Consultation_ResponsesDeadline = ""/>
            <#assign ConsultationDocPK = ""/>
            <#assign ConsultationADocUuid = ""/>
            <#assign ConsultationDocTitle  = ""/>
            <#assign ConsultationDocTitleField = ""/>
        ...
        <#if AID != 0 >
            ...
            <#assign ED_Consultation_Title = document.valueOf("//dynamic-element[@name='Consultation_Title']/dynamic-content/text()") />

            <#assign ED_Consultation_CPID = document.valueOf("//dynamic-element[@name='TextIDChangeProposal']/dynamic-content/text()") />
            <#assign ED_Consultation_TargetedAudience = document.valueOf("//dynamic-element[@name='Consultation_TargetedAudience']/dynamic-content") />

            <#assign ED_Consultation_ChooseQATemplate = document.valueOf("//dynamic-element[@name='Consultation_ChooseQATemplate']/dynamic-content/text()") />
            <#assign ED_Consultation_LinkToCPPage = document.valueOf("//dynamic-element[@name='TextLinkChangeProposalID']/dynamic-content/text()") />
            <#assign ED_Consultation_Documents = saxReaderUtil.createXPath("//root//dynamic-element[@name='TextDocumentConsultation']//dynamic-content//text()") />
            <#assign ED_Consultation_Document_Title = saxReaderUtil.createXPath("//root//dynamic-element[@name='Consultation_Document_Title']//dynamic-content//text()") />
        <#assign ConsultationNODE =  ED_Consultation_Documents.selectNodes(document) />
        <#assign ConsultationNODE2 = ED_Consultation_Document_Title.selectNodes(document) />

            <#assign cont = 0 />
            <#list ConsultationNODE as nodes>
                    <#if nodes.getText()?contains("classPK")>
                        <#assign ConsultationDoc = jsonFactoryUtil.createJSONObject(nodes.getText())>
                        <#assign ConsultationDocPK = ConsultationDoc.getString("classPK") />
                        <#assign ConsultationDoc = jsonFactoryUtil.createJSONObject(nodes.getText())>
                        <#assign ConsultationDocTitle = ConsultationDoc.getString("title") />
                        <#assign ConsultationDoc = jsonFactoryUtil.createJSONObject(nodes.getText())>
                        <#assign ConsultationADocUuid = ConsultationDoc.getString("uuid") />
                        <#assign ConsultationDocTitleField = ConsultationNODE2[cont].getText() />
                        <input type="hidden" class="fileInfoListItem" data-pk="${ConsultationDocPK}" data-name="${ConsultationDocTitle}" data-id="${ConsultationADocUuid}" data-doctitle="${ConsultationDocTitleField}"/>
                        <#assign cont++>
                    </#if>
                </#list>

            <#assign ED_Consultation_ResponsesDeadline = document.valueOf("//dynamic-element[@name='TextDate']//dynamic-content") />
            <#assign IA_CommentsDeadline_Data = getterUtil.getString(ED_Consultation_ResponsesDeadline)>
            ...
            <#if validator.isNotNull(IA_CommentsDeadline_Data)>
                ...
                <#assign ED_Consultation_ResponsesDeadline = dateUtil.getDate(IA_CommentsDeadline_DateObj, "yyyy-mm-dd", locale) />
                <#else> 
                <p>No date</p>
            </#if>
           
            <#assign targetAudienceSelector = saxReaderUtil.createXPath("//root//dynamic-element[@name='Consultation_TargetedAudience']//dynamic-content//option") />
            <#assign targetNodes = targetAudienceSelector.selectNodes(document) />
            <#assign valueTargetNodes><#list targetNodes as nodes>${nodes.getText()}<#sep>,</#sep></#list></#assign>
            <input type="hidden" id="selectedValuesForEdit" value="${valueTargetNodes}">
            <input type="hidden" id="filesToDelete" value="">
            <#assign edit = true />
                <#if ED_Consultation_Title = "">
                    <#assign ED_Consultation_Title = ja_title />
                </#if>
            <#else>
                <#assign edit = false />
        ...
        <br>
            ...
            <p>Title *<input type="text" id = "consultationTitle" name="consultationTitle" value="${articlleCustomTitle}"> </p>
           <#else>
             <p>Title *<input type="text" id = "consultationTitle" name="consultationTitle" value="${ED_Consultation_Title}"> </p>
            </#if>

            </div>
                <div><p>Consultation ID *<input type="text" id = "consultationId" name="consultationId" value="${ED_Consultation_CPID}"></p></div>

                <p><label for="targetedAudience">Targeted Audience *</label>
                <select name="select1" id="select1" required multiple size ="5"style=" height:300px;">
            ...
            <div > 
                ...
                <div > 
                <select name="select2" id="select2" required multiple size ="5"style=" height:300px;">
      ...
      <div>
         <p>Link to Change Proposal <input type="text" id="linkToCP" name="linkToCP" value="${ED_Consultation_LinkToCPPage}" /></p>
      </div>

       
      <div id="assurancePeriodDiv">
         <p id="assurancePeriod"> Response Deadline * <input id="dateFile" type="date" name="dateFile" value="${ED_Consultation_ResponsesDeadline}" required></p>
      </div>
...
<script>
    ...
    $('#select1, #select2').focus(function(evt) {let oEvt = evt || window.event; oEvt.target.blur();});
     ...
     <#if AID != 0>
        ...
        console.log("ArticleId ->" + ARTTICLEID);
            ...
            selectElement('selectQtpt', "${ED_Consultation_ChooseQATemplate}");
         </#if>
        ...
        let elementValuesArray = elementValues.trim().split(',');
   ...
   function highlight(element) {
    ...
    function selectElement(id, valueToSelect) {
        ...
        <#if ED_Consultation_TargetedAudience != "">
    ...
    function hideEditFileUpload(element) {
     ...
     function submitFile(){
         if ($("#select1").val() == "emptyOption" || $("#select1").val() == "") {
            ...
            }else if ($("#consultationId").val() == ""){
                    alert ("Consultation ID is required");
            }else if ($("#selectQtpt").val() == "emptyOption" || $("#selectQtpt").val() == "") {
                alert ("Question Temaplte is required field");
            }else if ($("#consultationTitle").val() == ""){
            alert ("Title is required");
    }
            else if ($("#consultationTitle").val() == "") {
                    alert ("Consultation title is required field");
             }else{
                submitIA();
    ...
    async function getValidName(fileName, alreadySelectedValidNames) {
      ...
      function uploadingFile() {
        ...
        var title = window.document.getElementById("consultationTitle").value;
        ...
        var cpId = window.document.getElementById("consultationId").value;
        ...
        <#if appRPK != 0 >
            Liferay.Service(
                '/cproposal.recformarticle/Update_CONSULTATION', {
                    appPK: ${appRPK},
                    Title: title,
                    TextIDChangeProposal: cpId,
                    Consultation_TargetedAudience: TargetAU,
                    Consultation_ChooseQATemplate: questionTPT,
                    TextLinkChangeProposalID: linkToCP,
                    TextDocumentConsultation: docJSON,
                    TextDate: date
                },
                function(obj) {
                    showResponse(obj);
        ...
        <#else>
            Liferay.Service(
                '/cproposal.recformarticle/create_CONSULTATION', {
                    Title: title,
                    TextIDChangeProposal: cpId,
                    Consultation_TargetedAudience: TargetAU,
                    Consultation_ChooseQATemplate: questionTPT,
